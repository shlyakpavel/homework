...
unsigned char process(element *&head) {
    float avg = 0; int num = 0;
  for (element* tmp = head; tmp->prev; tmp = tmp->prev){
      avg+= tmp->student.uspevaemost.average;
      num++;
  }
  avg = avg/num;
  ...
  element *el = head;
  while (el) {
    if (el->student.uspevaemost.average > avg) {
      el = el->prev;
    } else { // delete element
      if (el->prev)
